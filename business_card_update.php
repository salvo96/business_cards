...
if(isset($_POST["title"])&&isset($_POST["name"])&&isset($_POST["surname"])&&isset($_POST["email"])&&isset($_POST["phone"])&&isset($_POST["education_id"])&&isset($_POST["work_id"])&&isset($_POST["card_id"])){
    $title = $_POST["title"];
    $name = $_POST["name"];
    ...
    if($user_id_1 == $user_id_2)
        if($user_id == $user_id_1){
            $stmt = mysqli_prepare($conn, "UPDATE cards SET title = ?, name = ?, surname = ?, email = ?, phone = ?, education_experience_id = ?, work_experience_id = ? WHERE user_id = ? AND card_id = ?");
            mysqli_stmt_bind_param($stmt, "sssssiiii", $title, $name, $surname, $email, $phone, $education_id, $work_id, $user_id, $card_id);
